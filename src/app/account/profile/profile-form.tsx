...
export function ProfileForm({ userId, initialValues, isNew }: ProfileFormProps) {
  ...
  // After a successful save, sync the avatar URL field so re-render shows latest.
  useEffect(() => {
  ...
  async function handleAvatar(file: File) {
    ...
    if (!sb) {
      setUploadError("Storage isn't configured.");
      return;
    }
    if (file.size > 4 * 1024 * 1024) {
      setUploadError("Avatar is over 4MB — try a smaller image.");
      return;
    }
    setUploading(true);
    try {
    ...
    <form action={formAction} className="space-y-6">
      ...
      <div className="flex flex-col items-center gap-3 sm:flex-row sm:items-start sm:gap-6">
        ...
        <div className="flex-1">
          ...
          <label
            className={`inline-flex cursor-pointer items-center gap-2 border-4 border-ink bg-paper px-4 py-2 font-[family-name:var(--font-bangers)] tracking-widest text-ink shadow-graffiti hover:bg-acid-lime ${
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        />
        <Field
          label={isNew ? "Neighborhood *" : "Neighborhood"}
          name="neighborhood"
        ...
        <Field
          ...
          name="countryFlag"
          placeholder="🇺🇸"
          defaultValue={initialValues.countryFlag}
          maxLength={4}
        />
      </div>
      ...
      <Field
        label="Vibe tags"
        name="vibe"
        placeholder="graffiti, neon, portrait"
      ...
      {state && !state.ok && (
        <p className="border-2 border-ink bg-blood-orange p-3 font-[family-name:var(--font-bangers)] tracking-wide text-paper">
          {state.message}
        </p>
      )}
      {state?.ok && (
        <p className="border-4 border-ink bg-acid-lime p-3 font-[family-name:var(--font-bangers)] text-2xl tracking-widest text-ink shadow-graffiti">
          {state.message}
        </p>
      )}
    </form>
